...
import { LorenzoPoRAddressList__factory } from '../typechain-types';

const deployFn: DeployFunction = async (hre) => {
  const [ deployer ] = await ethers.getSigners();

  const LorenzoPoRAddrs = [
  ...
  const lorenzoPoRContract = await new LorenzoPoRAddressList__factory(deployer).deploy(LorenzoPoRAddrs, deployer.address);
  const lorenzoPoRAddr = await lorenzoPoRContract.getAddress();
  console.log("lorenzoPoRAddr address: ", lorenzoPoRAddr)
}
